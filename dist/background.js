...
function experimentMatches(settings          , granted                                      )           {
  return [
...
async function registerScripts(settings          , previous           )                {
...
async function apply(settings          , previous           )                {
...
function allowlistChange(previous          , host        , allowed         )           {
...
chrome.runtime.onMessage.addListener((message         , sender, reply) => {
  ...
  const task = serialize(async () => {
    ...
    if (request.type === "get-state") {
      ...
      const pauseUntil = Date.now() + (request.minutes          ) * 60 * 1000;
      ...
      settings = await change(previous => allowlistChange(previous, host, request.allowed           ));
      ...
      settings = await change(previous => allowlistChange(previous, request.host          , request.allowed           ));
