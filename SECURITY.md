...
AdAegis 0.4.3 is an early public build. It is not an audited security product.
...
- Timed pause uses the `alarms` permission so a 10-minute or 1-hour pause can end after the worker sleeps. While a timed pause is active, a second alarm refreshes the toolbar badge about once a minute so the remaining time stays visible.
- **Remove page access** in the popup turns those page features off and drops the grants.

Settings are stored locally and can only be changed from this extension's popup. Page scripts receive two booleans for that page (cleanup and YouTube), not your exception list. Restricting storage to trusted extension contexts is best-effort; if Chrome rejects that lock, settings still load and save.
...
| Player responses | Same-origin `/youtubei/v1/player`, `/get_watch`, `/player/ad_break`, `/next`, and Shorts reel watch JSON, success, no redirect, via `json()`, `text()`, `clone()`, `arrayBuffer()`, `blob()`, `body`, or XHR. Missing or `text/plain` content types still count. Nested `playerResponse` on those replies only. `JSON.parse` of the same shapes on the page. Also on search and channel pages while the experiment is on |
...
| Volume | At most 200 edited fetch/XHR player responses per video; `JSON.parse` cleanups do not count toward that cap. At most 256 top-level keys inspected |
...
| Pause | Turning the experiment off, hiding the page, or opening account, sign-in, billing, or embedded player URLs. Search and channels keep player filtering in place. Toggling the experiment back on during in-page navigation restores hooks for the next video even after a prior error stopped filtering |
