...
3. Hit a domain covered by the bundled rules; pause and try again. Pause 10 minutes and 1 hour: the badge should show remaining time and count down about once a minute, and protection should return when the time is up. Indefinite pause should show OFF and stay off across a browser restart.
...
5. With cleanup off, enable only YouTube filtering. The prompt should be YouTube-only, not Music. Reload YouTube, then play a watch video: preroll ads should be stripped, skipped, or seeked off the existing player. Open home, watch, and Shorts. Click from home to a video without a full reload; filtering should continue. Open search or a channel, then a video; the new video should still be filtered, including when the address has a trailing slash. Account pages must not keep those hooks.
6. Cause a player error or stall on one video, then click a different video without reloading. Filtering should start again for the new video. The same video should stay off until you reload or click something else. If filtering was off because of that error, turn the experiment off and on during navigation to a different video; hooks should return without a full reload.
...
11. On YouTube, turning the experiment off in the popup must restore hooks only after you turn it back on. Normal player controls must stay clickable. Skip must not fire on forms or links, or hammer a rotating button. A Skip control that uses HTML’s default button type, sits behind a hidden placeholder, or is in the player’s open shadow root should still be clicked. If Skip never appears during a preroll, the ad should still end.
