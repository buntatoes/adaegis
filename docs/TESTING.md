...
8. Toggle features and **Remove page access** on open pages. Scripts that were never injected still need a reload. If YouTube filtering was already injected, turning it off and on should restore hooks without a reload. Confirm grants are actually gone.
9. On YouTube, a playback error or stall must stay off until reload; a policy disable must not. Normal player controls must stay clickable. Skip must not fire on forms or links, or hammer a rotating button.
