import { normalize, hostname, validHost, exceptionRules, ALL_SITES, YOUTUBE_SITES, type Settings } from "./settings.js";
...
async function registerScripts(settings: Settings): Promise<void> {
  ...
  for (const script of desired) {
    if (current.some(existing => existing.id === script.id)) await chrome.scripting.updateContentScripts([script]);
